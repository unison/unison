...
sub get_table_info($) {
  ...
  while ( my $row = $sth->fetchrow_hashref() ) {
	...
	$tables{$fkst} = Unison::Schema::Table->new(
												namespace => $row->{fk_namespace},
												relation  => $row->{fk_relation}
											   ) unless exists $tables{$fkst};

	my $pkst = "$row->{pk_namespace}.$row->{pk_relation}";
	$tables{$pkst} = Unison::Schema::Table->new(
												namespace => $row->{pk_namespace},
												relation  => $row->{pk_relation}
											   ) unless exists $tables{$pkst};
