...
our @EXPORT = ();
our @EXPORT_OK =
  qw(get_p2_scores get_scop_pdb  display_points display_bars display_table %params );
...
my %params = (
    nclusters => 4,
    algorithm => "kcluster",
    associate => [ 'white', 'yellow', 'blue', 'red',
        'grey' ]    # in the order of increasing scores
);
...
"select m.pseq_id,m.pmodel_id from pmprospect m join pmsm_pmprospect s on m.pmodel_id=s.pmodel_id where s.pmodelset_id=$params{pmodelset_id}"
        )
      };
    ...
    @ps = map { $_->[0] } @{
        $u->selectall_arrayref(
            "select pseq_id from pseqset where pset_id=$params{pcontrolset_id}")
      };
    ...
    if (%ms) {
        my $sql =
          Unison::SQL->new()->table('paprospect')->columns($score)
          ->columns('pmodel_id')->columns('pseq_id')
...
#the 0 x 100 is a hack to force PG to choose a better(aka faster) execution plan!
          ->where(
            'pmodel_id in (' . join( ',', ( values %ms, (0) x 100 ) ) . ')' );
...
sub _plot_points() {
    ...
    $points_graph->set_legend( map { $$pmodel_pdb{$_} }
          sort keys %$pmodel_pdb );
...
sub display_bars($) {
    ...
    my ($png_fh) = @_;
    my (
        $cols,     $temp,     $mins,        $maxs, $err_mins,
        $err_maxs, $err_meds, $divider_min, $divider_max
    );
    ...
    foreach my $pseq ( sort { $$sp{$a} <=> $$sp{$b} } keys %$sp ) {
        if ( exists( $$scores{ $$cols[0] }{$pseq} ) ) {
            ...
            push @$err_maxs,
              (
                defined($max)
                ? ( $mo->query + $sd->query < $max
                    ? $mo->query + $sd->query
                    : $max )
                : undef
              );
            push @$err_meds,
              ( defined( $$scores{ $$cols[0] }{$pseq} ) ? $mo->query : undef );
...
sub _plot_bars {
    ...
    $bar_graph->set(
        ...
        title =>
'Scoring Range for Known and Known-NOT sequences against known Models',
        markers      => [1],
        marker_size  => 1,
        x_label_skip => ( $#{ $$data[0] } / 20 ) +
          1,    #skipping a label for every 20, X values.
        x_all_ticks   => 1,
        y_tick_number => 10,
        cumulate      => 1,
        error_bars    => 1
    ) or warn $bar_graph->error;
    $bar_graph->set_legend(
        '', 'Total Scoring Range',
        '', '1 Std. Dev Range',
        'Mean Score'
    );
    $bar_graph->set(
        dclrs => [ ( '', 'blue', '', 'lred', 'green', 'black', 'black' ) ] );
...
sub display_table($) {
    ...
    return "CLUSTERING OF THREADING SCORES FAILED!"
      if ( !defined($cluster_arr) );

    my $ret =
      '<STYLE>h1 {font-size:60%} </STYLE><FONT size="1"><table border=1><tr>';
    $ret .= '<th>seq/model</th>';
    ...
    foreach ( keys %$scores ) {
        ...
        if ( exists( $$pmodel_scop{$_} ) ) {
            $link =
"<a href=\"$scopURL$$pmodel_scop{$_}{'sunid'}\" title=\"$$pmodel_scop{$_}{'name'}:$$pmodel_scop{$_}{'descr'}\">$$pmodel_scop{$_}{'name'}</a>";
        }
        else {
            $link =
                "<a href=\"$pdbURL"
              . substr( $$pmodel_pdb{$_}, 0, 4 )
              . "\"title=\"$$pmodel_pdb{$_}>$$pmodel_pdb{$_}\"</a>";
        }
    ...
    foreach my $i ( sort { $$sp{$a} <=> $$sp{$b} } keys %$sp ) {
        ...
        if ( $i == 0 ) {
            $ret .= '</tr><tr>' x 4;
        }    #seperation between knowns and known-nots
        else {
            my $link =
                "<a href=\"pseq_summary.pl?pseq_id=$i\" title=\""
              . ( $u->best_annotation($i) || '' )
              . "\">$i</a>";
            $ret .=
                '</tr><tr><td height=10 ALIGN=center>' 
              . $link
              . '</td>';    #first col in each row is a pseq link
            ...
            foreach my $j ( keys %$scores ) {
                ...
                $ret .=
                  sprintf(
                    "<td height=10 ALIGN=center bgcolor=%s><h1 title=%.2f>%."
                      . ( $order == 1 ? 1 : 0 )
                      . "f</h1></td>",
                    $clr, $$scores{$j}{$i}, $$scores{$j}{$i} )
                  if ( defined( $$scores{$j}{$i} ) );
            }
        }
    }
    $ret .= '</tr></table></FONT>';
...
sub get_scop_pdb($) {
    ...
    my @pm_scop = @{
        $u->selectall_arrayref(
'select pmodel_id,sp,pdb,descr from scop_pmodel_v where pmodel_id in  ('
              . join( ',', keys %$scores )
              . ') order by pmodel_id'
        )
      };
    ...
    my @pm_pdb = @{
        $u->selectall_arrayref(
                'select pmodel_id,acc from pmprospect where pmodel_id in  ('
              . join( ',', keys %$scores )
              . ') order by pmodel_id'
        )
      };
...
sub compute_stats($) {
    ...
    my @sorted_scores =
      sort { $score eq 'raw' ? $a <=> $b : $b <=> $a }
      ( @{ $known_nots[0] }, @{ $known_nots[1] } );
    ...
    foreach my $s (@sorted_scores) {
        ...
        $stats_data->{ sprintf( "%.2f", 1 - $specificity ) }{$score} =
