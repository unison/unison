...
sub ssp_phd {
  ...
  my $sql = "select s.seq,p.confidence,p.h_confidence,p.e_confidence,p.c_confidence from psipred p join pseq s on p.pseq_id=s.pseq_id where p.pseq_id=$pseq_id and params_id=$params_id";
  ...
  my ($sfh, $sfn) = File::Temp::tempfile(DIR => '/tmp', SUFFIX=>".ssp", UNLINK=>1);
