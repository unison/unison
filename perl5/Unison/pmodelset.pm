...
sub get_pmodelset_name_by_pmodelset_id($$) {
    ...
    my (@rv) =
      $self->selectrow_array( 'select name from pmodelset where pmodelset_id=?',
        undef, $pmodelset_id );
...
sub pmodelset_si_pmodelset_id($$) {
    ...
    my ($rv) =
      $self->selectrow_array("select pmodelset_si_pmodelset_id ('$pmodelset')");
    return $rv;
}
...
sub pmodelset_last_updated($$) {
    ...
    if ( defined $_[2] ) {
        $self->do(
"update pmodelset set last_updated=now() where pmodelset_id=$pmodelset_id"
        );
    }
    my $sth =
      $self->prepare("select last_updated from pmodelset where pmodelset_id=?");
...
'select distinct ms.pmodelset_id,s.name from pmsm_pmhmm ms join pmodelset s on s.pmodelset_id=ms.pmodelset_id'
      );
    return @$a;
}
...
'select pmodelset_id from run where run_id=preferred_run_id_by_pftype( ? )',
        undef, $pfeature_name
    );
    return $id;
}
