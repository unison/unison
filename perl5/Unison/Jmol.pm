...
sub initialize{
  ...
  $stringio->print("jmolApplet([$self->{'width'}, $self->{'height'}], \"load $pdb_url; set frank off;spacefill off; wireframe off; cartoon on; color cartoon yellow; select $select_chain; restrict selected; center $select_chain;zoom 150;set echo off;set echo top left;font echo 18 serif;color echo white; echo $name;");
...
sub load{
  ...
  my $pdb_url = pdb_url($fn);
  return "load $pdb_url; set frank off;spacefill off; wireframe off; cartoon on; color cartoon yellow;select $select_chain; restrict selected; center $select_chain;zoom 150;set echo off;set echo top left;font echo 18 serif;color echo white; echo $name;";
}
