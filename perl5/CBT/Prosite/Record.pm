...
BEGIN
  {
  $RCSHeader = '$Header: /mnt/cvs/cbc/opt/lib/perl5/Prosite/Record.pm,v 0.5 1999/10/09 14:58:52 reece Exp $ ';
...
# Counts from NR
sub nTotal
  { $_[0]->NR =~ m/TOTAL=(\d+)\((\d+)\)/     && ($1,$2) }
sub nTruePositive
  { $_[0]->NR =~ m/POSITIVE=(\d+)\((\d+)\)/  && ($1,$2) }
sub nUnknownPositive
  { $_[0]->NR =~ m/UNKNOWN=(\d+)\((\d+)\)/   && ($1,$2) }
sub nFalsePositive
  { $_[0]->NR =~ m/FALSE_POS=(\d+)\((\d+)\)/ && ($1,$2) }
sub nFalseNegative
  { $_[0]->NR =~ m/FALSE_NEG=(\d+)/          && $1      }
sub nPartial
  { $_[0]->NR =~ m/PARTIAL=(\d+)/            && $1      }
...
sub taxo_range
  ...
  if (not exists $self->{'taxo_range'})
	{
	($self->{'taxo_range'}) = 
	  exists $self->{'CC'} ? $self->{'CC'} =~ m%/TAXO-RANGE=(.{5})% 
	: undef;
	}
...
sub max_repeat
  ...
  if (not exists $self->{'sites'})
	{
	($self->{'max_repeat'}) = 
	  exists $self->{'CC'} ? $self->{'CC'} =~ m%/MAX-REPEAT=(\d+)% 
	: undef;
	}
...
sub sites
  ...
  if (not exists $self->{'sites'})
	...
	if (exists $self->{'CC'} and $self->{'CC'} =~ m%/SITE=\d+,[^;]+%)
	  {
	  my $x = $self->{'CC'};
	  @{$self->{sites}} = map {s%,%/%;$_} $self->{'CC'}=~m%/SITE=(\d+,[^;]+)%g;
	  }
	}
  return @{$self->{'sites'}};
  }
...
sub match
  ...
  my $count; $count++ while ($seq =~ m/$re/g);
  return $count;
  }
