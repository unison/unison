...
sub think {
    ...
    if ( $s == 0 ) { print( STDERR "I can think, apparently\n" ); return; }

    throw MyModule::Exception(
        {
            error  => '% exception iv from hash',
    ...
    throw MyModule::Exception( { detail => 'exception wo/error string' } )
      if $s == 5;
