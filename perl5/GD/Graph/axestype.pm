...
sub initialise {
    ...
    while ( my ( $key, $val ) = each %Defaults ) { $self->{$key} = $val }
...
sub set {
    ...
    for ( keys %args ) {
        /^tick_length$/ and do {
            $self->{x_tick_length} = $self->{y_tick_length} = $args{$_};
            delete $args{$_};
            next;
        };
        /^long_ticks$/ and do {
            $self->{x_long_ticks} = $self->{y_long_ticks} = $args{$_};
            delete $args{$_};
            next;
        };
        /^overwrite$/ and do {
...
sub setup_right_boundary {
    ...
    if ( !$self->{rotate_chart} ) {
        ...
        if ( $self->{two_axes} ) {
            $max_len = $self->{y_label_len}[2]
              if $self->{y_label_len}[2] > $max_len;
        }
...
sub setup_x_step_size_v {
    ...
    if ( defined $s->{x_tick_number} ) {
        my $delta =
          ( $s->{right} - $s->{left} ) / ( $s->{x_max} - $s->{x_min} );
        ...
        $s->{x_step} =
          ( $s->{right} - $s->{left} ) / ( $s->{_data}->num_points + 1 );
        $s->{x_offset} = $s->{left};
    }
}
...
sub setup_x_step_size_h {
    ...
    if ( defined $s->{x_tick_number} ) {
        my $delta =
          ( $s->{bottom} - $s->{top} ) / ( $s->{x_max} - $s->{x_min} );
        ...
        $s->{x_step} =
          ( $s->{bottom} - $s->{top} ) / ( $s->{_data}->num_points + 1 );
        $s->{x_offset} = $s->{top};
    }
}
...
sub setup_coords {
    ...
    undef( $s->{x_tick_number} )
      if !defined $s->{x_max}
          || !defined $s->{x_min}
          || $s->{x_max} == $s->{x_min};

    $s->{rotate_chart}
      ? $s->setup_x_step_size_h()
    ...
    for my $t ( 0 .. $self->{y_tick_number} ) {
        ...
        for my $axis ( 1 .. ( $self->{two_axes} + 1 ) ) {
            my $label =
              $self->{y_min}[$axis] +
              $t *
              ( $self->{y_max}[$axis] - $self->{y_min}[$axis] ) /
              $self->{y_tick_number};
            ...
            if ( defined $self->{y_number_format} ) {
                $label =
                  ref $self->{y_number_format} eq 'CODE'
...
sub create_x_labels {
    ...
    if ( defined $self->{x_tick_number} ) {
        ...
        foreach my $t ( 0 .. $self->{x_tick_number} ) {
            ...
            if ( defined $self->{x_number_format} ) {
                $label =
                  ref $self->{x_number_format} eq 'CODE'
    ...
    $self->{x_label_height} =
      $self->{x_labels_vertical} ? $maxlen : $self->{xafh};
    $self->{x_label_width} =
      $self->{x_labels_vertical} ? $self->{xafh} : $maxlen;
}
...
sub draw_axes {
    ...
    if ( $self->{box_axis} ) {
        $self->{graph}
          ->filledRectangle( $l + 1, $t + 1, $r - 1, $b - 1, $self->{boxci} )
          if $self->{boxci};
    ...
    for my $t ( 0 .. $self->{y_tick_number} ) {
        for my $axis ( 1 .. ( $self->{two_axes} + 1 ) ) {
            ...
            if ( $self->{y_long_ticks} ) {
                $self->{graph}
                  ->line( $x, $self->{bottom}, $x, $self->{top}, $self->{fgci} )
                  unless ( $axis - 1 );
            }
            else {
                $self->{graph}->line( $x, $y, $x, $y - $self->{y_tick_length},
                    $self->{fgci} );
            }
...
sub draw_y_ticks_v {
    ...
    for my $t ( 0 .. $self->{y_tick_number} ) {
        ...
        for my $axis ( 1 .. ( $self->{two_axes} + 1 ) ) {
            ...
            if ( $self->{y_long_ticks} ) {
                $self->{graph}
                  ->line( $x, $y, $x + $self->{right} - $self->{left},
                    $y, $self->{fgci} )
                  unless ( $axis - 1 );
            }
            else {
                $self->{graph}->line( $x, $y,
                    $x + ( 3 - 2 * $axis ) * $self->{y_tick_length},
                    $y, $self->{fgci} );
            }
...
sub draw_x_ticks_h {
    ...
    for ( my $i = 0 ; $i < $self->{_data}->num_points ; $i++ ) {
        ...
        next
          if (  !$self->{x_all_ticks}
            and ( $i - $self->{x_tick_offset} ) % $self->{x_label_skip}
            and $i != $self->{_data}->num_points - 1 );
        ...
        if ( $self->{x_ticks} ) {
            if ( $self->{x_long_ticks} ) {
                ...
                $self->{graph}->line( $x, $y, $x + $self->{x_tick_length},
                    $y, $self->{fgci} );
            }
        }
        ...
        # CONTRIB Damon Brodie for x_tick_offset
        next
          if ( $i - $self->{x_tick_offset} ) % ( $self->{x_label_skip} )
              and $i != $self->{_data}->num_points - 1;
        ...
        if ( $self->{x_ticks} ) {
            if ( $self->{x_long_ticks} ) {
                ...
                if ( $self->{x_tick_length} >= 0 ) {
                    ...
                    $self->{graph}
                      ->line( $x, $self->{bottom} - $self->{x_tick_length},
...
sub draw_data {
    ...
    if ( $self->{bar_width} ) {
        ...
        my $bar_space =
          $chart_width / ( $n_bars + 1 ) / ( $self->{overwrite} ? 1 : $n_sets );
...
sub draw_values {
    ...
    for ( my $dsn = 1 ; $dsn <= $self->{_data}->num_sets ; $dsn++ ) {
        my @values = $self->{_data}->y_values($dsn)
          or return $self->_set_error( "Impossible illegal data set: $dsn",
            $self->{_data}->error );
        ...
        for ( my $i = 0 ; $i < @values ; $i++ ) {
            ...
            if ( defined $self->{values_format} ) {
                $value =
                  ref $self->{values_format} eq 'CODE'
...
sub _correct_y_min_max {
    ...
    return ( $min, $max )
      unless $self->isa("GD::Graph::bars")
          or $self->isa("GD::Graph::area");
...
sub set_max_min {
    ...
    if ( $self->{two_axes} ) {
        my $min_range_1 =
          defined( $self->{min_range_1} )
          ? $self->{min_range_1}
          : $self->{min_range};
        my $min_range_2 =
          defined( $self->{min_range_2} )
          ? $self->{min_range_2}
          : $self->{min_range};
        (
            $self->{y_min}[1], $self->{y_max}[1], $self->{y_min}[2],
            $self->{y_max}[2], $self->{y_tick_number}
          )
          = _best_dual_ends(
        ...
        if ( $self->{cumulate} ) {
            ...
            if ( $self->{error_bars} ) {
                ...
                ( $y_min, $y_max ) =
                  $data_set->get_min_max_y( $data_set->num_sets );
            }
...
sub _best_dual_ends {
    ...
    my ( $best_min_1, $best_max_1, $best_min_2, $best_max_2, $best_n,
        $best_fit ) = ( $min_1, $max_1, $min_2, $max_2, 1, 1e10 );
      ...
      )
    {
        ...
        my $fit =
          _measure_interval_fit( $bmin_1, $min_1, $max_1, $bmax_1 ) +
          _measure_interval_fit( $bmin_2, $min_2, $max_2, $bmax_2 );

        next if $best_fit < $fit;

        (
            $best_min_1, $best_max_1, $best_min_2,
            $best_max_2, $best_n,     $best_fit
        ) = ( $bmin_1, $bmax_1, $bmin_2, $bmax_2, $n, $fit );
    }
...
sub _get_bottom {
    ...
    if ( $self->{cumulate} && $ds > 1 ) {
        my ( $left, $pvalue );
        if ( $self->{error_bars} ) {
            $pvalue = $self->get_y_max( $ds - 1, $np );
        }
        else {
            $pvalue = $self->{_data}->get_y_cumulative( $ds - 1, $np );
        }
...
{                   # return [x, y] in pixel coords
    ...
    # XXX use_axis
    my $y_min =
      ( $self->{two_axes} && $i == 2 ) ? $self->{y_min}[2] : $self->{y_min}[1];

    my $y_max =
      ( $self->{two_axes} && $i == 2 ) ? $self->{y_max}[2] : $self->{y_max}[1];
    ...
    if ( defined( $self->{x_min_value} ) && defined( $self->{x_max_value} ) ) {
        ...
        $ret_x =
          ( $self->{x_tick_number} ? $self->{x_offset} : $origin ) +
          $x * $self->{x_step};
    }
    my $ret_y =
        $self->{rotate_chart}
...
sub setup_legend {
    ...
    if ( $lg_pos eq 'R' ) {
        ...
        if ( $lg_align eq 'T' ) {
            ...
            $self->{lg_ys} =
              $self->{height} - $self->{b_margin} - $self->{lg_y_size};
        }
        else    # default 'C'
        {
            my $height =
              $self->{height} - $self->{t_margin} - $self->{b_margin};
    ...
    else        # 'B' is the default
    {
        ...
        ( !defined( $self->{lg_cols} ) )
          and $self->{lg_cols} = int( $width / $self->{lg_el_width} );

        $self->{lg_cols} = _min( $self->{lg_cols}, $num );

        $self->{lg_rows} =
          int( $num / $self->{lg_cols} ) +
          ( ( $num % $self->{lg_cols} ) ? 1 : 0 );
        ...
        if ( $lg_align eq 'R' ) {
            $self->{lg_xs} =
              $self->{width} - $self->{r_margin} - $self->{lg_x_size};
        }
        elsif ( $lg_align eq 'L' ) {
...
sub draw_legend_marker    # data_set_number, x, y
{
    ...
    $g->filledRectangle(
        $x, $y,
        $x + $s->{legend_marker_width},
        $y + $s->{legend_marker_height}, $ci
    );

    $g->rectangle(
        $x, $y,
        $x + $s->{legend_marker_width},
        $y + $s->{legend_marker_height},
        $s->{acci}
    );
}
