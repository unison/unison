...
($GD::Graph::points::VERSION) = '$Revision$' =~ /\s([\d.]+)/;
...
sub draw_data_set {
    ...
    for ( my $i = 0 ; $i < @values ; $i++ ) {
        ...
        {
            ( $xp, $yp ) =
              $self->val_to_pixel( $self->{_data}->get_x($i), $values[$i],
                $ds );
        }
        else {
            if ( $self->{_data}->get_x($i) == 0 ) {
                $dsci = $self->set_clr( _rgb('black') );
                $type = 10;
            }
            else {
                $dsci = $self->set_clr( $self->pick_data_clr($ds) );
                $type = $self->pick_marker($ds);
            }
...
sub marker    # $xp, $yp, $type, $colourindex
{
  ...
  MARKER: {
        ...
        ( $mtype == 9 ) && do {    # Horizontal line
            $self->{graph}->line( $l, $yp, $r, $yp, $mclr );
            last MARKER;
        };
        ( $mtype == 10 ) && do {    # vertical line
            $self->{graph}->line( $xp, $t, $xp, $b, $mclr );
            last MARKER;
        };
    }
...
sub draw_legend_marker {
    ...
    my $ms =
      _min( $self->{legend_marker_height}, $self->{legend_marker_width} );
