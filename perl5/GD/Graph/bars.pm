...
sub draw_data {
    ...
    unless ( $self->{no_axes} ) {
        ...
        if ( $self->{rotate_chart} ) {
            $self->{graph}->line(
                $self->{zeropoint}, $self->{top}, $self->{zeropoint},
                $self->{bottom},    $self->{fgci}
            );
        }
        else {
            $self->{graph}->line(
                $self->{left},      $self->{zeropoint}, $self->{right},
                $self->{zeropoint}, $self->{fgci}
            );
        }
    }
...
sub _top_values {
    ...
    if ( $self->{cumulate} ) {
        ...
        for my $i ( 0 .. $data->num_points - 1 ) {
            if ( $self->{error_bars} ) {
                push @topvalues, $self->get_y_max( 4, $i );
            }
            else {
...
sub _draw_shadow {
    ...
    if ( $value >= 0 ) {
        if ( $self->{rotate_chart} ) {
            $self->{graph}
              ->filledRectangle( $l, $t + $bsd, $r - $bsd, $b + $bsd, $bsci );
        }
        else {
            $self->{graph}
              ->filledRectangle( $l + $bsd, $t + $bsd, $r + $bsd, $b, $bsci );
        }
    }
    else {
        if ( $self->{rotate_chart} ) {
            $self->{graph}
              ->filledRectangle( $l + $bsd, $t, $r + $bsd, $b, $bsci );
        }
        else {
            $self->{graph}
              ->filledRectangle( $l + $bsd, $b, $r + $bsd, $t + $bsd, $bsci );
        }
    }
}
...
sub draw_data_set_h {
    ...
    my @values = $self->{_data}->y_values($ds)
      or return $self->_set_error( "Impossible illegal data set: $ds",
        $self->{_data}->error );
...
sub draw_data_set_v {
    ...
    if ( $self->{error_bars} ) {
        $self->calc_error_bar_spacing($ds);
    }
    ...
    for ( my $i = 0 ; $i < @values ; $i++ ) {
        ...
        if ( $self->{error_bars} and $$topvalues[$i] < 0 ) {
            my $tmp = $bottom;
            $bottom = $t;
            $t      = $tmp;
        }
...
sub calc_error_bar_spacing {
    my $self = shift;
    my $ds   = shift;

    if ( $ds <= 2 ) {
        $self->{bar_spacing} = $self->{x_step} - 0.8;
    }
    else {
        $self->{bar_spacing} = $self->{x_step} - 2.8;
    }
}
