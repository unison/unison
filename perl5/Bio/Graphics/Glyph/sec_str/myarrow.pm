...
#  warn join ' ',%$self;
    ...
    $self->arrowhead( $gd, $x1, $center, $a2, -1 )
      if $sw && !$trunc_left;    # west arrow

    for ( my $i = $x2 ; $i > $x1 + 6 ; ) {
        $self->arrowhead( $gd, $i, $center, $a2 - 1, +1 )
          if $ne && !$trunc_right;    # east arrow
        $self->arrowhead( $gd, $i - 1, $center, $a2 - 1, +1 )
          if $ne && !$trunc_right;    # east arrow
        $self->arrowhead( $gd, $i - 2, $center, $a2 - 1, +1 )
          if $ne && !$trunc_right;    # east arrow
        $i = $i - 8;
    }
