...
sub _initialize {
	...
	warn "WARNING:Kiran Mukhyala's bug fix version of bioperl modules";
...
sub _read_GenBank_Species {
	...
	if ( /^SOURCE\s+(.*)/o ) {
	    ...
	    my $line = $1;
            ...
            # organism line
	    # Kiran Mukhyala: unless the line contains an open bracket, it might be closed
	    # in the next line ex: NP_852617
	    # SOURCE      Peanut witches'-broom phytoplasma
            # ORGANISM    Peanut witches'-broom phytoplasma
            #            Bacteria; Firmicutes; Mollicutes; Acholeplasmatales;
            #            Acholeplasmataceae; Candidatus Phytoplasma; 16SrII (Peanut WB
            #            group).
	    my $scopes = 0;
	    $scopes++ foreach grep {/\(/} split(//,$line);
	    $scopes-- foreach grep {/\)/} split(//,$line);

            if ($line !~ /[;\.]$/ and not $scopes) {
                $sci_name .= ' '.$line;
            }
            else {
                $class_lines .= $line;
    ...
    ####### copied patch from http://bugzilla.open-bio.org/show_bug.cgi?id=2288 ############
    ...
    # spaces
    # example in NP_052127
    # problem line: Acetobacteraceae; Acetobacter; Acetobacter subgen. Acetobacter.
    $class_lines=~s/\.\s*$//;
