...
BEGIN
	...
	select into a_id QA.palias_id from pseqalias QA
		...
		where QA.pseq_id=q_id and QA.iscurrent=TRUE and OA.porigin_id=porigin_id_lookup(''SPDI'')
		order by alias desc
		limit 1;
	return a_id;
END;';
...
BEGIN
	select into a_id QA.palias_id from pseqalias QA
		join paliasorigin OA on OA.palias_id=QA.palias_id
		where QA.pseq_id=q_id and QA.iscurrent=TRUE and OA.porigin_id=porigin_id_lookup(''SPDI'') and alias~''^UNQ''
		limit 1;
	return a_id;
END;';
...
BEGIN
	select into a_id QA.palias_id from pseqalias QA
		join paliasorigin OA on OA.palias_id=QA.palias_id
		where QA.pseq_id=q_id and QA.iscurrent=TRUE and OA.porigin_id=porigin_id_lookup(''SPDI'') and alias~''^PRO''
		limit 1;
	return a_id;
END;';
...
BEGIN
	select into a_id QA.palias_id from pseqalias QA
		join paliasorigin OA on OA.palias_id=QA.palias_id
		where QA.pseq_id=q_id and QA.iscurrent=TRUE and OA.porigin_id=porigin_id_lookup(''SPDI'') and alias~''^DNA''
		limit 1;
	return a_id;
END;';
