...
--
-- $Id: pmap.sql,v 1.3 2006/01/24 18:48:51 mukhyala Exp $
-- -----------------------------------------------------------------------------
...
CREATE UNIQUE INDEX pmap_hsp_unq ON pmap_hsp USING btree (params_id,genasm_id, chr, strand, gstart, gstop, pseq_id, pstart, pstop);
CREATE INDEX pmap_hsp_p_lookup ON pmap_hsp USING btree (params_id,pseq_id, pstart, pstop);
CREATE INDEX pmap_hsp_g_lookup ON pmap_hsp USING btree (params_id,genasm_id, chr, strand, gstart, gstop);
...
ALTER TABLE ONLY pmap_alnhsp
    ADD CONSTRAINT pmap_aln_id_exists FOREIGN KEY (aln_id) REFERENCES pmap_aln(aln_id) ON UPDATE CASCADE ON DELETE CASCADE INITIALLY DEFERRED;
ALTER TABLE ONLY pmap_alnhsp
...
--   arguments: genasm_id,chr,strand,gstart,gstop,pseq_id,pstart,pstop
--   returns: pmap_hsp_id
--
CREATE OR REPLACE FUNCTION ins_pmap_hsp(integer, text, char, integer, integer, integer, integer, integer, integer) RETURNS integer AS '
DECLARE
...
CREATE OR REPLACE VIEW PMAP_V AS
	SELECT a.params_id, a.genasm_id, h.pseq_id, ah.aln_id, min(h.pstart) AS pstart, max(h.pstop) AS pstop, count(*) AS exons, sum(h.pstop - h.pstart + 1) AS aln_length, (sum(h.pstop - h.pstart + 1)::double precision / q.len::double precision * 100::double precision)::integer AS pct_cov, a.ident, ((a.ident / sum(h.pstop - h.pstart + 1))::double precision * 100::double precision)::integer AS pct_ident, h.chr, h.strand, min(h.gstart) AS gstart, max(h.gstop) AS gstop
   FROM pmap_hsp h
   JOIN pmap_alnhsp ah ON h.hsp_id = ah.hsp_id
   JOIN pmap_aln a ON ah.aln_id = a.aln_id
   JOIN pseq q ON h.pseq_id = q.pseq_id
  GROUP BY a.params_id, a.genasm_id, h.pseq_id, ah.aln_id, h.chr, h.strand, a.ident, q.len
...
CREATE OR REPLACE VIEW PMAP_ALN_V AS
	SELECT a.params_id, ah.aln_id, h.pseq_id, h.pstart, h.pstop, h.gstart, h.gstop, a.ident, h.genasm_id, h.chr, h.strand
   FROM pmap_hsp h
   JOIN pmap_alnhsp ah ON h.hsp_id = ah.hsp_id
   JOIN pmap_aln a ON ah.aln_id = a.aln_id
