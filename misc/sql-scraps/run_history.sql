...
BEGIN
	...
	if not FOUND then
		...
		insert into run_history (pseq_id,params_id,porigin_id,pmodelset_id,failed,ran_on) values (q,p,o,m,f,z);
	end if;
	return z;
END;';

comment on function upd_run_history(integer,integer,integer,integer,boolean) is 'inserts/updates run_history for the given pseq_id,params_id,porigin_id,pmodelset_id,failed); returns timestamp';
...
comment on function upd_run_history(integer,integer,integer,integer) is 'equivalent to upd_run_history(pseq_id,params_id,porigin_id,pmodelset_id,NULL)';
