...
function jmolChangeStructureLink(script, label, id, title) {
  _jmolInitCheck();
  if (id == undefined || id == null)
    id = "jmolLink" + _jmol.linkCount;
  if (label == undefined || label == null)
  ...
  var t = "<span id=\"span_"+id+"\""+(title ? " title =\"" + title + "\"":"")+"><a name='" + id + "' id='" + id + 
          "' href='javascript:_jmolClick(" + scriptIndex + _jmol.targetText + ");' onMouseover='_jmolMouseOver(" + scriptIndex +
          ");return true;' onClick='pdbid=text; ' onMouseout='_jmolMouseOut()' " +
          _jmol.linkCssText + ">" + label + "</a></span>";
  if (_jmol.debugAlert)
    alert(t);
  return _jmolDocumentWrite(t);
...
function jmolSelectPosition(pos, labe, colour, targetSuffix) {
  ...
  var selPos = Number(seq_str[pdbid][pos]);
  if ( selPos == '') {
  ...
  var script = "save orientation init_orient; select within(10.0, "+selPos+chain+"); hbonds 0.1; ssbonds on; select "+selPos+chain+"; wireframe 0.5;colour  "+colour+";select "+selPos+chain+" and *.CA; label "+labe+":"+selPos+"; colour label black; ";
...
function jmolSelectRegion(posone, postwo, labe, colour, targetSuffix) {
  ...
  */
  var script = "save orientation init_orient; select "+start+"-"+end+chain+"; colour cartoon "+colour+"; select "+label_pos+chain+" and *.CA; label "+labe+"; colour label black; ";
