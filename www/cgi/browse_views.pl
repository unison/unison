...
my $u = $p->{unison};
$p->add_footer_lines('$Id: browse_views.pl,v 1.16 2006/08/16 21:21:19 mukhyala Exp $ ');
...
sub do_search {
  ...
  $s_sql->columns('pseq_locus(X.pseq_id,3,45) as locus') if (exists $v->{a_locus});
  $s_sql->columns('best_alias(pseq_locus_rep(X.pseq_id,3,45)) as locus_rep') if (exists $v->{a_locus_rep});
  $s_sql->columns('best_annotation(X.pseq_id)') if (exists $v->{a_best_annotation});
  $s_sql->columns('X.*');
  $s_sql->columns('Q.added::date') if (exists $v->{a_added});
  $s_sql->columns("(select as_set(distinct 'UNQ'||unqid) from gg_unq_pro_dna_pseq_v where pseq_id=X.pseq_id group by unqid order by unqid) as unqs") if (exists $v->{a_unq});
  $s_sql->columns("(select as_set(distinct 'PRO'||proid) from gg_unq_pro_dna_pseq_v where pseq_id=X.pseq_id group by proid order by proid) as pros") if (exists $v->{a_pro});
  $s_sql->columns("(select as_set(distinct(chip||':'||probe_id)) from pseq_probe_v P where P.pseq_id=X.pseq_id) as probes") if (exists $v->{a_probes});
...
sub build_form {
  ...
  return (
		  ...
		  $p->checkbox(-name => 'a_best_annotation',
					   ...
					   -checked => $v->{a_best_annotation} ),
		  '<br>',
		  $p->checkbox(-name => 'a_probes',
					   -label => 'microarray probs',
					   -checked => $v->{a_probes}),
		  '</td>',
