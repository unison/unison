...
my %cvs = %{
    $u->selectall_hashref(
        'select cv_id,name,descr,sql,order_by from canned_views', 'cv_id' )
  };

print $p->render( 'Canned Views', build_form($p), do_search($p) );
...
sub do_search {
    ...
    my $sql;
    try {
        my $s_sql = Unison::SQL->new()->table("($cv->{sql}) X");

        if ( exists $v->{r_species} ) {
            $s_sql->join('palias A on X.pseq_id=A.pseq_id')
        ...
        $s_sql->columns("pseq_locus(X.pseq_id,3,$v->{pmap_params_id}) as locus")
          if ( $v->{a_locus} );
        $s_sql->columns(
"best_alias(pseq_locus_rep(X.pseq_id,3,$v->{pmap_params_id})) as locus_rep"
        ) if ( $v->{a_locus_rep} );
        $s_sql->columns('best_annotation(X.pseq_id)')
          if ( $v->{a_best_annotation} );
        $s_sql->columns('X.*');
        $s_sql->columns('Q.added::date') if ( $v->{a_added} );
        $s_sql->columns(
"(select as_set(distinct 'UNQ'||unqid) from pseq_sst_v where pseq_id=X.pseq_id) as unqs"
        ) if ( $v->{a_unq} );
        $s_sql->columns(
"(select as_set(distinct 'PRO'||proid) from pseq_sst_v where pseq_id=X.pseq_id) as pros"
        ) if ( $v->{a_pro} );
        $s_sql->columns(
"(select as_set(distinct(chip||':'||probe_id)) from pseq_probe_mv P where P.pseq_id=X.pseq_id and P.params_id= $v->{pmap_params_id} and P.genasm_id=3) as probes"
        ) if ( $v->{a_probes} );
...
# restrict menu to cv_id>0 unless a cv_id<0 is being displayed (usually power users)
    @cvs = grep { $_ > 0 } @cvs
      unless ( defined $v->{cv_id} and $v->{cv_id} < 0 );

    my %cvlabels =
      map { $_ => sprintf( "%s (canned view %d)", $cvs{$_}->{name}, $_ ) } @cvs;

    my %sl = map { $_->[0] => "$_->[1] ($_->[3]; $_->[2])" } @{
        $u->selectall_arrayref(
            "select tax_id,common,latin,gs from tax.spspec
