...
$p->add_footer_lines('$Id: pseq_pahmm.pl,v 1.21 2007/01/09 17:43:02 mukhyala Exp $ ');
...
try {
  ...
  if (not defined $v->{pmodelset_id}) {
	$v->{pmodelset_id} =
	  $u->preferred_pmodelset_id_by_pftype('HMM')
		|| $ps[0]->[0];
  }
...
EOJS
  ...
  print $p->render ("HMM alignments for Unison:$v->{pseq_id}",
					...
					'<br>parameters: ', $p->popup_menu(-name => 'params_id',
													   ...
													   -default => "$v->{params_id}"),
		                        '&nbsp;',
		                        'modelsets: ', $p->popup_menu(-name => 'pmodelset_id',
													   -values => [map {$_->[0]} @ms],
													   -labels => \%ms,
...
EOSQL
  ...
  while ( my $r = $sth->fetchrow_hashref() ) {
	my $name = sprintf('<a tooltip="%s" href="http://pfam.janelia.org/family?id=%s">%s (%s)</a>',
					   $r->{descr}, $r->{name}, $r->{name}, $r->{acc});
	my @row;

	if ( not $p->is_public_instance() ) {
	  my $ckbox = "<input type=\"checkbox\" name=\"profiles\" value=\"$r->{name}\">";
	  my $aln = sprintf('<a href="javascript:update_emb_hmm_alignment(%d,%d,%d,\'%s\')">show</a>', 
						$v->{pseq_id}, $v->{params_id}, $v->{pmodelset_id}, $r->{name});
	  push( @row, "$ckbox &nbsp; &nbsp; $aln" );
	}
