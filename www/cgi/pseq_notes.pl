...
$p->add_footer_lines('$Id: pseq_notes.pl,v 1.6 2005/06/18 00:16:45 rkh Exp $ ');
...
my @cols = ( [ 'User', 'login' ], [ 'Added', 'added' ], [ 'Note', ], );
my $ob = { login => 'login', added => 'added desc' }->{ $v->{sort} };
my @f = map { $_->[0] } @cols;
my %colnum =
  map { $cols[$_]->[1] => $_ } grep { defined $cols[$_]->[1] } 0 .. $#cols;
$sql =
qq/select login,date_trunc('second',added) as "added",note from pnote where pseq_id=$v->{pseq_id} order by $ob/;
my $ar;
