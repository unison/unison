...
use lib "$FindBin::Bin/../perl5", "$FindBin::Bin/../perl5-prereq",
  "$FindBin::Bin/../../perl5", "$FindBin::Bin/../../../perl5";
...
my $scoplinkfmt =
  '<A HREF="' . $scopURL . '/search.cgi?sunid=%d" TOOLTIP="%s">%s</A>';
sub scoplink ($$);
...
$p->add_footer_lines(
    '$Id: pseq_paprospect.pl,v 1.38 2006/06/26 17:36:37 rkh Exp $ ');
...
{
    $sql->where(
"pmodel_id in (select pmodel_id from pmsm_pmprospect where pmodelset_id=$v->{pmodelset_id})"
    );
}
...
foreach my $row ( @{$feats} ) {
    ...
    my $aln =
      "<input type=\"checkbox\" name=\"templates\" " . "value=\"$row->{acc}\">";

    # build structure link
    my $strxlink = $row->{acc};
    if ( -f "$pdbDir/pdb" . substr( $row->{acc}, 0, 4 ) . ".ent" ) {
        my $url =
"p2cm.pl?pseq_id=$v->{pseq_id};viewer=$v->{viewer};params_id=$v->{params_id};templates=$row->{acc}";
        $strxlink = sprintf(
            '<span tooltip="%s"><a href="%s">%s</a><span>',
    ...
    for ( my $i = 0 ; $i <= $#{ $row->{scop} } ; $i++ ) {
        ...
        push( @scop,
                scoplink( $scopr->{clid}, $scopr->{clname} )
              . '&nbsp;<b>&gt;</b>&nbsp;'
...
if ( $v->{offset} > 0 ) {
    ...
    push(
        ...
        sprintf(
            "<a href=\"%s\">%s</a>",
            $p->make_url( { offset => 0 }, @statevars ),
            '<span tooltip="first (full) page" class="button">|&lt;&lt;</span>'
        ),
        sprintf(
            "<a href=\"%s\">%s</a>",
            $p->make_url( { offset => $no }, @statevars ),
...
if ( $N - 1 - $v->{offset} > 0 ) {
    ...
    push(
        ...
        sprintf(
            "<a href=\"%s\">%s</a>",
            $p->make_url( { offset => $no }, @statevars ),
            '<span tooltip="ahead one page" class="button">&gt</span>'
        ),
        sprintf(
            "<a href=\"%s\">%s</a>",
            $p->make_url( { offset => $N - $v->{limit} }, @statevars ),
            '<span tooltip="last (full) page" class="button">&gt;&gt;|</span>'
        )
    );
}
...
print $p->render(
    ...
    $p->tip(
        'clicking some column headings will dynamically sort by that column'),
