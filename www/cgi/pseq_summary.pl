...
try {
    print $p->render( "Unison:$v->{pseq_id} Summary", summary_group($p), );
}
catch Unison::Exception with {
    $p->die(shift);
};
...
sub summary_group ($) {
    ...
    my ($netphos_digest) = $u->selectrow_array(
        'select digest from pseq_features_netphos_v where pseq_id=?',
        undef, $v->{pseq_id} );
...
'select chr||band from pseq_cytoband_v where pseq_id=? and params_id=48',
    ...
    my $strx = join(
        ', ',
        map {
            $p->tooltip( pdbc_rcsb_link( $_->[0] ),
                sprintf( "%s: %s (I:%d%%,C:%d%%)", @$_ ) )
          } @{
            $u->selectall_arrayref(
'select distinct substr(pdbc,1,4),descr,pct_ident,pct_coverage from papseq_pdbcs_mv where q_pseq_id=?',
                undef, $v->{pseq_id}
            )
          }
    );
...
# try best human annotation first, otherwise get best annotation for any species
    ...
    @GOs =
      grep { $_->{evidence} =~ m/IDA|IPI|IPM|IGI|IEP|TAS|IC|ISS|IGC|RCA/ } @GOs;
    ...
    if (@GOs) {
        $go_text = join(
            ...
            '<span class="note">'
              . sprintf(
'See the <a href="pseq_functions.pl?pseq_id=%d">Functions</a> tab for evidence, PubMed references, and NCBI %s.',
                $v->{pseq_id},
                $p->tooltip(
                    'GeneRIFs', "NCBI's Gene References Into Function"
                )
              )
              . '</span>'
        );
...
'See the <a href="pseq_paliases.pl?pseq_id=%d">Aliases</a> tab for all aliases of this sequence.',
        ...
        $domain_digest,
        '<br>',
        'Phosphorylation sites (pos;probability): ',
        $netphos_digest,
        '<br>',
        features_graphic($p),
        '<span class="note">'
          . sprintf(
'See the <a href="pseq_features.pl?pseq_id=%d">Features</a> tab for additional sequence features.',
            $v->{pseq_id} )
          . '</span>',
        '</td></tr>',
        "\n",
...
sub aliases ($) {
    ...
    return map {
        $_->[0] . ':' . $p->tooltip( alias_link( $_->[1], $_->[0] ), $_->[2] )
    } @$ar;
}
