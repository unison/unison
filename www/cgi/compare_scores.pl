...
##
        ...
        if ( exists $v->{submit} ) {
            ...
            if ( scalar keys %$scores < 1 ) { $v->{tag} = "NO DATA"; }
                ...
                if ( $v->{Plot} eq 'Clustered' ) {
                    $v->{tag} =
                      Unison::Utilities::compare_scores::display_table($u);
                }
                elsif ( $v->{Plot} eq 'Scatter' ) {

                    my ( $data, $map ) =
                      Unison::Utilities::compare_scores::display_points(
                        $png_fh);
                    $v->{tag} =
                      sprintf( '<IMG SRC="%s" usemap="#scattermap">%s',
                        $png_urn, $map->imagemap( $png_fn, $data ) );
                }
        ...
        my @params = @{
            $u->selectall_arrayref(
' select params_id,name from params where name like \'Prospect2 default%\''
            )
          };
...
'Sequences from the selected model set will be deleted from the control set plotting the scores'
        ...
        $p->tooltip(
            'Select Score ',
            'svm and raw score are part of the Prospect2 threading method'
        ),
        '</th>',
        '<th width=20%>',
        $p->tooltip(
            'Select Plots ',
'All plots show scores of sequences(known and known-not) against models (from the selected model set)'
        ),
        '</th>', '</tr>',
        "\n",
...
sub _footnote {
    ...
    if ( $plot eq 'Clustered' ) {
        $ret .=
"Note : Data for the selected model set are seperated from the rest by an inserted row.";
    }
    elsif ( $plot eq 'Scatter' ) {
        $ret .=
"Note : Data for the selected model set are seperated from the rest by a dashed line in the plot. Some sequence_ids might not be labelled due to lack of space";
    }
    elsif ( $plot eq 'Range' ) {
        $ret .=
"Note : Data for the selected model set are seperated from the rest by a black line in the plot. Some sequence_ids might not be labelled due to lack of space";
    }
    return $ret;
}
main();
