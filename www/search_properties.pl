...
my (@db_pri) = sort
  qw( GenenGenes UniProtKB/Swiss-Prot UniProtKB/TrEMBL Incyte Proteome RefSeq
...
my $s_sql =
  Unison::SQL->new()->table('palias A')->columns('distinct A.pseq_id');

if ( exists $v->{o_sel} ) {
    my @origin_id =
      map { $u->origin_origin_id_by_origin($_) } $p->param('o_sel');
    $s_sql->where(
        'A.origin_id in (' . join( ',', sort { $a <=> $b } @origin_id ) . ')' );
}
...
"select pmodel_id from pmsm_pmprospect where pmodelset_id=$v->{al_ms_sel}"
        )
      };
...
sub spit_form {
    ...
    my %go = map { $_->[0] => "$_->[1] (GO:$_->[0])" } @{
        $u->selectall_arrayref(
            "select go_id,alias from gong.alias 
    ...
    my %sl = map { $_->[0] => "$_->[1] ($_->[2])" } @{
        $u->selectall_arrayref(
            "select tax_id,common,latin from tax.spspec
...
'<br>NOTE: selecting species filtering will eliminate sequences whose species is not explicity denoted',
        ...
        ' with svm >= ',
        $p->popup_menu(
            -name    => 'al_prospect_svm',
            -values  => [qw(13 12 11 10 9 8 7 6 5)],
            -default => '9'
        ),
        ' using parameter set ',
        $p->popup_menu(
            -name    => 'al_prospect_params_id',
            -values  => [qw(1)],
            -default => '1'
        ),
        '<hr>',
